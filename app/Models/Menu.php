...
class Menu
{
	public static function get_menu($params = ['where' => ['is_shown = 1']], $join = [])
	{
	...
	{
		$locale = !empty($locale) ? $locale : 'ua';

		$menu_items = self::get_menu(['where' => ['parent_id = 0', 'is_shown = 1'], 'order_by' => 'order_num']);
		$menu_sub_items = self::get_menu(['where' => ['parent_id != 0', 'is_shown = 1'], 'order_by' => 'order_num']);
// все для адміна ( тимчасове рішення )
		if (User::get_current_user_id() == 1) {
			$menu_sub_items[] = [
				"id" => "0",
				"name" => "all bugs and suggestions",
				"link" => "/info/all",
				"access_id" => "0",
...
//				"element_id" => "updates"
		...
		if (!empty($menu_items)) {
			foreach ($menu_items as &$menu_item) {
				$menu_item['element_id'] = $menu_item['name'];
				$menu_item['name'] = lang_('Menu.' . $menu_item['name'], $locale);
				if (!empty($menu_sub_items)) {
					foreach ($menu_sub_items as &$menu_sub_item) {
						if ($menu_sub_item['parent_id'] == $menu_item['id']) {
							$menu_sub_item['element_id'] = $menu_sub_item['name'];
							$menu_sub_item['name'] = lang_('Menu.' . $menu_sub_item['name'], $locale);
...
//		return $menu;
	...
	public static function select_query($join_keys = [])
	{
		return [
			'table' => ['fb' => 'fin_menu'],
			'columns' => [
				'id', 'name', 'link', 'access_id', 'icon', 'parent_id', 'order_num'
			],
			'columns_with_alias' => [
//				'column' => 'alias'
			],
